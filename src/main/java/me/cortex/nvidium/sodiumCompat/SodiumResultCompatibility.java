...
public class SodiumResultCompatibility {
    ...
    private static int[] computeRanges(int[] counts) {
        var ranges = new int[ModelQuadFacing.COUNT * 2];
    ...
    private static void packageSectionGeometry(int formatSize, NativeBuffer output, short[] outOffsets, ChunkBuildOutput result, Vector3i min, Vector3i max) {
        ...
        if (translucentData != null && Nvidium.config.translucency_sorting_level == TranslucencySortingLevel.SODIUM) {
            var translucentRanges = computeRanges(translucentData.getVertexCounts());
            for (int i = 0; i < ModelQuadFacing.COUNT; i++) {
                offset += copyRange(
                        translucentData,
            ...
            var ranges = computeRanges(translucentData.getVertexCounts());
        ...
        var solidRanges = solid != null ? computeRanges(solid.getVertexCounts()) : null;
        var cutoutRanges = cutout != null ? computeRanges(cutout.getVertexCounts()) : null;
