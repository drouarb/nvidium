...
void main() {
    ...
    #ifdef USE_NV_FRAGMENT_SHADER_BARYCENTRIC
        VA0 = attributesData[(quadId<<2)+TRI_INDICIES.x];
        VAp = attributesData[(quadId<<2)+TRI_INDICIES.y];
        VA2 = attributesData[(quadId<<2)+TRI_INDICIES.z];
    ...
    #else
        float lodBias = hasMipping(VA0)?0.0f:-4.0f;
